...
const Orbit = ()=>{
  ...
  //three.js camera
  const [perspectiveCamera,setPerspectiveCamera] = useState(null)
  const [orthographicCamera,setOrthographicCamera] = useState(null)
  ...
  const onPanFail = (event) =>{
    ...
    let {nativeEvent} = event;
    if(viewSize!=null && perspectiveCamera!=null){
      const pos = new Vector2(2*nativeEvent.absoluteX/size -1, -2*nativeEvent.absoluteY/size+1)
      const raycaster = new Raycaster();
      raycaster.setFromCamera(pos,perspectiveCamera)
      const intersects = raycaster.intersectObject( Object.values(components.current),false)
